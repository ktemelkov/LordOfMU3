...
#pragma pack(pop)
	...
	#define _ApiHook_MakePtr(Type, Base, Offset) ((Type)(DWORD_PTR(Base) + (DWORD_PTR)(Offset)))
	 ...
	 * \brief 
	 */
	bool ReleaseMem(HANDLE hProcess, PVOID pRemMem)
	{
		InitNtMemApi(NtApi)
		
		PVOID pvMemAddr = pRemMem;
		ULONG ulNullSize = 0;
		return NtApi.NtFreeVirtualMemory(hProcess, &pvMemAddr, &ulNullSize, MEM_RELEASE) >= 0;
	}
	...
	DWORD_PTR FindPatchPoint(HANDLE hProcess, DWORD_PTR pFuncAddress, DWORD_PTR pCallback)
	{
		...
		TestResultRet(!(buff[0] == 0xE9 && buff[5] == 0xEb && buff[6] == 0xF9), FindPatchPoint(hProcess, *((DWORD*)(buff+1)) + pFuncAddress + 5, pCallback))
	...
	bool DisasmAndCopyTramp(BYTE* buff, DWORD_PTR pRemFuncAddr, CPatchInfo* pPatchInfo, CPatchInfo* pRemPatch, DWORD& dwIdx, ULONG ulSizeToProtect)
	{
		...
		for (dwIdx = 0; dwIdx < ulSizeToProtect; )
		{
			...
			StringUtil::MemCopy(trampBuff + dwIdx, (void*)ci.code, dwInstrSize);
			StringUtil::MemCopy(trampBuffO + pPatchInfo->m_dwPatchSize, (void*)ci.code, dwInstrSize);

			if (trampBuff[dwIdx] == 0xE9 || trampBuff[dwIdx] == 0xE8) // fix far relative jumps/calls
			{				
				DWORD jmp = *((DWORD*)(ci.code + 1));
				DWORD dwDstAddr = jmp + (DWORD)ci.code + 5;
			...
			{	
				BYTE jmp = *((BYTE*)(ci.code + 1));
				DWORD dwDstAddr = (char)jmp + (DWORD)ci.code + 2;
	 ...
	 * \brief 
	 */
	bool RemoteUnPatch(HANDLE hProcess, DWORD_PTR ppOrigProcAddr)
	{
		if (!ppOrigProcAddr)
			return false;
		...
		TestResult(ReadProtectedMem(hProcess, ppOrigProcAddr, &pRemPatch, 4));

		if (pRemPatch == 0)
			return true;

		TestResult(ReadProtectedMem(hProcess, (DWORD)pRemPatch, &info, sizeof(info)));		
		TestResult(WriteProtectedMem(hProcess, info.m_dwProcAddr, info.m_abOriginalInstr, info.m_dwPatchSize));

		ReleaseMem(hProcess, pRemPatch);

		pRemPatch = 0;
		TestResult(WriteProtectedMem(hProcess, ppOrigProcAddr, &pRemPatch, 4));
	 ...
	 * \brief 
	 */
	bool RemoteFreeOriginallCallData(HANDLE hProcess, DWORD_PTR ppOrigProcAddr)
	{
		if (!ppOrigProcAddr)
			return false;
	...
	bool PatchRetn(DWORD_PTR pFuncAddr, DWORD_PTR pCallback)
	{
		...
		if (dwRetAddr == 0 || (dwRetAddr - (DWORD)pFuncAddr + dwRetSize) < 5)
			return false;


		ci.code = (BYTE*)pFuncAddr;
		ci.codeLen = 1024;

		DWORD dwCopyAddr = (DWORD)pFuncAddr;
		...
		while (1)
		{
			...
			int diff = (int)(dwRetAddr + dwRetSize) - (int)((DWORD)ci.code);

			if (diff < 5)
				break;

			dwCopyAddr = (DWORD)ci.code;
			ci.code += dwInstrSize;
			ci.codeLen -= dwInstrSize;
		}


		if ((int)(dwRetAddr + dwRetSize - dwCopyAddr) < 5)
			return false;
		...
		DWORD dwIdx = 0;
		ci.code = (BYTE*)dwCopyAddr;
		ci.codeLen = 1024;

		while ((DWORD)ci.code < dwRetAddr)
		{
			distorm_decompose(&ci, decodedInstructions, 1, &decodedInstructionsCount);
		...
		DWORD dwJump = GetJMP((DWORD)(trampBuff + dwIdx), (DWORD)pCallback);

		StringUtil::MemCopy(trampBuff + dwIdx + 1, &dwJump, 4);
		
		StringUtil::MemCopy(trampBuff + dwIdx + 5, (void*)dwRetAddr, dwRetSize);

		dwJump = GetJMP(dwCopyAddr, (DWORD)trampBuff);
	 ...
	 * \brief 
	 */
	void ReplaceFnCall(HMODULE hMod, FARPROC* ppfn,  FARPROC* ppfnRepl)
	{
		if (!hMod)
			hMod = (HMODULE)ProcessUtil::GetLocalProcessBaseAddress();

		if (!hMod)
			return;

		PIMAGE_DOS_HEADER pDOSHeader = (PIMAGE_DOS_HEADER)hMod;
		PIMAGE_NT_HEADERS pNTHeader = _ApiHook_MakePtr(PIMAGE_NT_HEADERS, pDOSHeader, pDOSHeader->e_lfanew);
		...
		{			
			if ((h_sec[i].Characteristics & IMAGE_SCN_CNT_CODE) == IMAGE_SCN_CNT_CODE)
			{
				...
				_CodeInfo ci;
				ci.code = (const BYTE*)pPhysicalAddress;
				ci.codeLen = (int)h_sec[i].SizeOfRawData;
				ci.codeOffset = 0;
				ci.dt = dt;
				ci.features = DF_NONE;
				...
				while (dwSize < h_sec[i].SizeOfRawData)
				{
					distorm_decompose(&ci, decodedInstructions, 1, &decodedInstructionsCount);
					...
					if (dwInstrSize == 6)
					{
						if (*((BYTE*)ci.code) == 0xFF && *((BYTE*)ci.code+1) == 0x15)
						{
							DWORD dwAddrCall;
							StringUtil::MemCopy(&dwAddrCall, (BYTE*)ci.code + 2, 4);

							if (dwAddrCall == (DWORD)ppfn)
							{
								DWORD dwWrite = (DWORD)ppfnRepl;
								WriteProtectedMem((HANDLE)-1, (DWORD)(ci.code+2), &dwWrite, 4);
							}
						}
					}
					else if (dwInstrSize == 5)
					{
						if (*((BYTE*)ci.code) == 0xE8)
						{
							DWORD dwAddress;
							StringUtil::MemCopy(&dwAddress, (BYTE*)ci.code + 1, 4);

							DWORD dwDst = (DWORD)dwAddress + (DWORD)ci.code + 5;

							if (dwDst == (DWORD)*ppfn)
							{
								Beep(3000,50);

								DWORD dwWrite = (DWORD)*ppfnRepl - (DWORD)ci.code - 5;
								WriteProtectedMem((HANDLE)-1, (DWORD)(ci.code+1), &dwWrite, 4);
							}
						}
					}
