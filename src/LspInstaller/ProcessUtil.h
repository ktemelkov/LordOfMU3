...
namespace ProcessUtil
{
	...
	typedef ULONG (__stdcall* LdrUnloadDllPtr)(HANDLE);
	...
	typedef BOOL (__stdcall* VirtualProtectExPtr)(HANDLE hProcess, LPVOID lpAddress, SIZE_T dwSize, DWORD flNewProtect, PDWORD lpflOldProtect);	
	...
	// gdi32.dll
	typedef BOOL (WINAPI* SetPixelFormatPtr)(HDC hdc, int iPixelFormat, const PIXELFORMATDESCRIPTOR *ppfd);
	typedef BOOL (WINAPI* SwapBuffersPtr)(HDC hdc);
