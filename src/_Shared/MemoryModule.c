...
#define MM_IMAGE_OFFSET 0x1000
//#define MM_IMAGE_OFFSET 0


typedef struct {
	PIMAGE_NT_HEADERS headers;
	unsigned char *codeBase;
	unsigned char *memBlockStart;
	HMODULE *modules;
	int numModules;
	int initialized;
...
FinalizeSections(PMEMORYMODULE module)
{
	...
	for (i=0; i<module->headers->FileHeader.NumberOfSections; i++, section++) {
		...
		protect = PAGE_EXECUTE_READWRITE;//ProtectionFlags[executable][readable][writeable];
...
BuildImportTable(PMEMORYMODULE module)
{
	...
	if (directory->Size > 0) {
		...
		for (; !IsBadReadPtr(importDesc, sizeof(IMAGE_IMPORT_DESCRIPTOR)) && importDesc->Name; importDesc++) {
			...
			if (module->modules == 0)
				module->modules = (HMODULE*)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, (module->numModules+1)*(sizeof(HMODULE)));
			else
				module->modules = (HMODULE*)HeapReAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, module->modules, (module->numModules+1)*(sizeof(HMODULE)));
 ...
 * \brief 
 */
HMEMORYMODULE __stdcall MemoryLoadLibrary(const void *data, LPVOID lpReserved, BOOL fWipeHeader)
{
	PMEMORYMODULE result;
	PIMAGE_DOS_HEADER dos_header;
	PIMAGE_NT_HEADERS old_header;
	unsigned char *code, *headers, *memblockStart;
...
//    if (code == NULL) {
		...
		dwImageOffset = 0; //0x1000;
		dwNewSize = old_header->OptionalHeader.SizeOfImage + dwImageOffset;
...
//	}
	...
	result->codeBase = code + dwImageOffset;
	result->memBlockStart = code;
	result->numModules = 0;
	result->modules = NULL;
	result->initialized = 0;

	code = result->codeBase;
	memblockStart = result->memBlockStart;

	// XXX: is it correct to commit the complete memory region at once?
    //      calling DllEntry raises an exception if we don't...
	VirtualAlloc(memblockStart,
		dwNewSize,
		MEM_COMMIT,
		PAGE_READWRITE);
	...
	if (result->headers->OptionalHeader.AddressOfEntryPoint != 0) {
		...
		successfull = (*DllEntry)((HINSTANCE)code, DLL_PROCESS_ATTACH, lpReserved);
		if (!successfull) {
			goto error;
		}
	...
	if (fWipeHeader)
	{
		...
		iFillSize = ((PIMAGE_DOS_HEADER)code)->e_lfanew - sizeof(IMAGE_DOS_HEADER) + 4;
...
FARPROC __stdcall MemoryGetProcAddress(HMEMORYMODULE module, const char *name)
{
...
void __stdcall MemoryFreeLibrary(HMEMORYMODULE mod)
{
	int i;
