...
BOOL CMuWindow::SubclassWindow2(HWND hWnd)
{
	...
	DWORD dwOldProt = 0;
	VirtualProtect((LPVOID)pProc, 16, PAGE_EXECUTE_READWRITE, &dwOldProt);
	...
	m_cWndProcTramp.Patch(pfnWndProc, pProc);
	m_pfnSuperWindowProc = m_cWndProcTramp.GetProc();
	m_hWnd = hWnd;
