...
//			m_fEnabled = FALSE;
	...
	else if (pkt == CCharRespawnPacket::Type())
	{
		...
		if (m_fAfkIsOn && (abs((int)m_bX0 - (int)m_bX) > 2 || abs((int)m_bY0 - (int)m_bY) > 2))
		{
			m_fAfkIsOn = false;
			m_fEnabled = FALSE;
		}
...
WORD CAutoKillFilter::KillNext(WORD wLastTarget)
{
	...
	if (wObject != 0)
	{
		...
		if (!fSuspended)
		{
			...
			KillObject(wObject, wSkill, arrDir[dx][dy]);
		}
 ...
 * \brief 
 */
void CAutoKillFilter::KillObject(WORD wId, WORD wSkill, BYTE dir)
{
	if (wSkill == 0)
	{
	...
	{
		GetProxy()->send_packet(CMassiveSkillPacket(LOBYTE(wSkill), m_bX, m_bY, dir, wId));
	}
}
